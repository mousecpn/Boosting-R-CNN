...
class LoadImageFromSUODAC:
    ...
    def __init__(self,
        ...
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        self.domain_list_init(domain_file)
        self.train = train

    def domain_list_init(self,domain_file):
        import os
        if domain_file is None:
            return
        self.domain_list = []
        txt_list = os.listdir(domain_file)
        for domain in txt_list:
            txt_file = os.path.join(domain_file, domain)
            f = open(txt_file, 'r')
            names = list(f.readlines())
            names = list(map(str.strip, names))
            f.close()
            self.domain_list.append(names)

        return

    def find_domain(self,path):
        path = path.split('/')[-1]
        path = path.split('.')[0]
        for i in range(len(self.domain_list)):
            if path in self.domain_list[i]:
                return i
        return 0
    ...
    def __call__(self, results):
        ...
        results['ori_shape'] = img.shape
        results['img_fields'] = ['img']
        style_idx = self.find_domain(filename)
        if self.train:
            domain_label = torch.zeros(len(self.domain_list))
            domain_label[style_idx] = 1
